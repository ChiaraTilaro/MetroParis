...
class Controller:
    ...
    def handleCreaGrafo(self,e):
        ...
        self._view.lst_result.controls.append(
            ft.Text(f"Il grafo è costituito da {self._model.get_numnodi()} nodi")
        )
        self._view.lst_result.controls.append(
            ft.Text(f"Il grafo è costituito da {self._model.get_numarchi()} archi")
        )
    ...
    def handleCercaRaggiungibili(self,e):
        if self._fermataPartenza is None:
            self._view.lst_result.controls.clear()
            self._view.lst_result.controls.append(
                ft.Text("Attenzione scegliere una stazione di partenza!")
            )
            self._view.update_page()
            return
        nodes = self._model.getBFSNodesEdges(self._fermataPartenza)
        self._view.lst_result.controls.clear()
        self._view.lst_result.controls.append(
    ...
    def handleTrovaPercorso(self , e):
        if self._fermataPartenza is None or self._fermataArrivo is None:
            self._view.lst_result.controls.clear()
            self._view.lst_result.controls.append(
                ft.Text("Attenzione necessario selezionare fermate di partenza e di arrivo" ,
                        color="red")
            )
            self._view.update_page()
            return
        ...
        self._view.lst_result.controls.clear()
        self._view.lst_result.controls.append(
                ft.Text(f"Ho trovato un cammino fra {self._fermataPartenza} e {self._fermataArrivo} che impiega {totTime} minuti " ,
                        color="green")
            )
        self._view.lst_result.controls.append(
